...
class Navigation {
   ...
   */
  isActive = () => document.body.classList.contains(ACTIVE_CLASS);

  toggleNav = (incActive: boolean | undefined) => {
    const { body } = document;
    const active: boolean = typeof incActive !== 'boolean' ? !this.isActive : incActive;

    body.classList.toggle(ACTIVE_CLASS, active);

    this.selectStuff(TOGGLES).forEach((el) => el.classList.toggle(VISIBLE_CLASS, active));
  ...
  mobileMenu = () => {
    let mobile = false;
    const openers = document.querySelectorAll(OPENERS);
    ...
    // if mobile
    if (mobile) {
      const closers = document.querySelectorAll(CLOSERS);
